  ...
  >
    ...
    >
      ...
      >
        ...
        >
          <i class="uil-multiply" @click="onclose"></i>
        </button>
      </div>

      <div class="py-4 px-2 mt-12">
        <div class="flex items-center mt-1" v-for="comment in comments">
          <img
            @click="goIn4(comment.User.username, comment.User.id)"
            class="w-6 h-6 rounded-full mr-2"
            :src="comment.User.avatar"
            alt="Avatar"
          />
          <div
            class="px-2 py-2 border rounded-md text-gray-700 text-sm bg-gray-100"
          ...
          <div
            class="px-2 py-2 text-gray-700 text-sm"
            v-if="(!showedit, idcomment === comment.id)"
          >
            <textarea
            ...
            >
              ...
              >
                <li class="py-2 px-1 flex items-center hover:bg-gray-100">
                  ...
                  <a @click="deletecomment(comment.id)" class="block px-4 py-2"
                    >Xóa</a
                  >
                </li>
                <li
                  class="py-2 px-1 flex items-center hover:bg-gray-100"
  ...
  methods: {
    ...
    async getcomment() {
      try {
        ...
        this.comments = result.data.filter(
          (item) => item.Post.id === this.postId
        );
      } catch (error) {
        console.log(error);
      }
