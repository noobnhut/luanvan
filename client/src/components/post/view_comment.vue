    ...
    <div class="fixed w-full h-full top-0 left-0 flex items-center justify-center z-50 ">
        ...
        <div
            ...
            <div
                ...
                <h5 class="text-lg font-semibold flex-grow">Bài viết của {{ username }}</h5>
                <button type="button"
            ...
            <div class="py-4 px-2 mt-12">
                <div class="flex items-center mt-1" v-for="comment in comments" >
                    ...
                    <div class="ml-2" :class="getclass(comment.User.id)">
                        ...
                        <div id="dropdownHover"
                            ...
                            <ul class="py-2 text-sm text-gray-700 dark:text-gray-200 cursor-pointer">
                                ...
                                <li class="py-2 px-1 flex items-center  hover:bg-gray-100">
                                    ...
                                    <a @click="editcomment(comment.id)" class="block  px-4 py-2  ">Chỉnh sửa</a>
                                </li>
                            </ul>
                        </div>
                    </div>
            ...
            <div class="modal-footer py-3 px-4 absolute bottom-0 left-0 right-0 ">
                <div class="flex items-center mt-2">
                    ...
                    <input class="w-full px-4 py-2 border rounded-full text-gray-700 focus:outline-none" type="text"
                        ref="comment" placeholder="Thêm bình luận..." v-on:keyup.enter="comment">
                </div>
            </div>
        </div>
    </div>
    ...
    methods: {
        ...
        async getcomment() {
            try {
                ...
                this.comments = result.data.filter((item) => item.Post.id === this.postId)
            } catch (error) {
                console.log(error)
